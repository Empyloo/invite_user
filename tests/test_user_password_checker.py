...
def test_is_password_set_true(mock_connect, mock_db_url, mock_email):
    ...
    assert is_password_set(db_url="mock_db_url", email="test@example.com") == True
    mock_cursor.execute.assert_called_once_with("SELECT encrypted_password FROM auth.users WHERE email = %s", ("test@example.com",))
...
def test_is_password_set_false(mock_connect, mock_db_url, mock_email):
    ...
    assert is_password_set(db_url="mock_db_url", email="test@example.com") == False
    mock_cursor.execute.assert_called_once_with("SELECT encrypted_password FROM auth.users WHERE email = %s", ("test@example.com",))
